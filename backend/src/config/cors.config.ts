...
const isOriginAllowed = (origin: string | undefined, allowedOrigins: string[]): boolean => {
  ...
  return allowedOrigins.some(allowedOrigin => {
    ...
    if (config.node_env === 'production') {
      ...
      return origin.endsWith(`.${allowedDomain}`) && origin.startsWith('https://');
    }
...
export const setupCors = (app: INestApplication): void => {
  ...
  const corsConfig: CorsConfig = {
    ...
    methods: config.node_env === 'development' 
  ...
  app.use((req: any, res: any, next: any) => {
    ...
    if (req.method !== 'GET' && config.node_env === 'production') {
      if (!referer || !isOriginAllowed(referer, allowedOrigins)) {
        console.warn(`Suspicious request without valid referer from IP: ${req.clientIp}`);
      }
    }
