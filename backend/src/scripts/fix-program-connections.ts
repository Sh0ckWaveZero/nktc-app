...
async function fixProgramConnections() {
  try {
    ...
    console.log(
      `Programs without Department: ${programsWithoutDepartment}/${totalPrograms} (${Math.round(
        (programsWithoutDepartment / totalPrograms) * 100,
      )}%)`,
    );
    console.log(
      `Students without Program: ${studentsWithoutProgram}/${totalStudents} (${Math.round(
    ...
    for (const program of programsToFix) {
      ...
      if (matchedDepartment) {
        ...
        programDeptFixCount++;
        console.log(
          `Connected program "${program.name || program.id}" to department "${
    ...
    for (const [index, student] of studentsToFix.entries()) {
      if (index % 100 === 0) {
        console.log(
          `Progress: ${index}/${studentsToFix.length} students processed`,
        );
      }

      let matchingProgram = null;
      const studentDeptId =
        student.departmentId || student.classroom?.departmentId;
    ...
    console.log(
      `Students without Program: ${studentsWithoutProgramAfter}/${totalStudents} (${Math.round(
        (studentsWithoutProgramAfter / totalStudents) * 100,
      )}%)`,
    );

    // Calculate improvement
    const programImprovement =
      programsWithoutDepartment - programsWithoutDepartmentAfter;
    const studentImprovement =
      studentsWithoutProgram - studentsWithoutProgramAfter;

    console.log(`\nImprovement summary:`);
    console.log(
      `- Fixed program-department connections: ${programImprovement} (${Math.round(
        (programImprovement / programsWithoutDepartment) * 100,
      )}%)`,
    );
    console.log(
      `- Fixed student-program connections: ${studentImprovement} (${Math.round(
        (studentImprovement / studentsWithoutProgram) * 100,
      )}%)`,
    );
  } catch (error) {
