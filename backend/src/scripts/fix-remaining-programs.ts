...
async function fixRemainingProgramConnections() {
  try {
    console.log(
      '=== Starting Fix for Remaining Program-Department Connections ===\n',
    );
    ...
    console.log(
      `Found ${programsWithoutDepartment.length} of ${totalPrograms} programs without department connections`,
    );

    if (programsWithoutDepartment.length === 0) {
      console.log(
        'All programs are already connected to departments. Nothing to do!',
      );
      return;
    }
    ...
    for (const program of programsWithoutDepartment) {
      ...
      if (program.name && program.name.includes('เทคนิคยานยนต์')) {
        manualMappings[program.id] = departments.find(
          (d) => d.name && d.name.includes('ช่างยนต์'),
        )?.id;
      } else if (program.name && program.name.includes('ไฟฟ้า')) {
        manualMappings[program.id] = departments.find(
          (d) => d.name && d.name.includes('ไฟฟ้า'),
        )?.id;
      } else if (
        program.name &&
        program.name.includes('เทคโนโลยีอิเล็กทรอนิกส์ (ม.6)')
      ) {
        // Specific mapping for เทคโนโลยีอิเล็กทรอนิกส์ (ม.6) that preserves the full name
        manualMappings[program.id] = departments.find(
          (d) => d.name && d.name.includes('อิเล็กทรอนิกส์'),
        )?.id;
      ...
      if (manualMappings[program.id]) {
        console.log(
          `  Found manual mapping to department: "${
            departments.find((d) => d.id === manualMappings[program.id])?.name
          }"`,
        );
        continue;
      }
      ...
      for (const dept of departments) {
        const deptWords = [
          ...
          ...(dept.description
            ? dept.description.toLowerCase().split(/\s+/)
            : []),
        ];
      ...
      if (bestScore >= 2 && bestMatch) {
        ...
        console.log(
          `  Found fuzzy text match to department: "${bestMatch.name}" (score: ${bestScore})`,
        );
      } else {
        ...
        if (studentsWithThisProgram.length > 0) {
          ...
          for (const student of studentsWithThisProgram) {
            if (student.departmentId) {
              deptCounts[student.departmentId] =
                (deptCounts[student.departmentId] || 0) + 1;
            }
          }
          ...
          if (mostCommonDeptId) {
            ...
            const deptName = departments.find(
              (d) => d.id === mostCommonDeptId,
            )?.name;
            console.log(
              `  Inferred department "${deptName}" for program "${program.name}" from ${maxCount} students`,
            );
          }
        }
      }
    ...
    for (const [programId, departmentId] of Object.entries(manualMappings)) {
      try {
        ...
        const program = programsWithoutDepartment.find(
          (p) => p.id === programId,
        );
        const department = departments.find((d) => d.id === departmentId);

        console.log(
          `Connected program "${program?.name || programId}" to department "${
            department?.name || departmentId
          }"`,
        );
      } catch (error) {
        console.error(
          `Error connecting program ${programId} to department ${departmentId}:`,
          error,
        );
      }
    }
    ...
    console.log(
      `Programs without Department: ${programsWithoutDepartmentAfter}/${totalPrograms} (${Math.round(
        (programsWithoutDepartmentAfter / totalPrograms) * 100,
      )}%)`,
    );

    // Calculate improvement
    const improvement =
      programsWithoutDepartment.length - programsWithoutDepartmentAfter;

    console.log(`\nImprovement summary:`);
    console.log(
      `- Fixed program-department connections: ${improvement} of ${
        programsWithoutDepartment.length
      } (${Math.round(
        (improvement / programsWithoutDepartment.length) * 100,
      )}%)`,
    );
  } catch (error) {
