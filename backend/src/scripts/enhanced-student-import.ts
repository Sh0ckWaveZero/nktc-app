  ...
  try {
    ...
    for (const item of workSheetsFromFile[0].data.filter(
      ...
      try {
        ...
        if (!studentId || !firstName) {
          console.log(`⚠️ Skipping record with missing essential data: ${item}`);
          continue;
        }
        ...
        if (!level) {
          console.log(`⚠️ Warning: Could not find or create level for ${levelName}`);
        }
        ...
        if (departmentName) {
          ...
          if (!department) {
            console.log(`⚠️ Warning: Could not find or create department for ${departmentName}`);
          }
        }
        ...
        if (programName && level) {
          program = await findOrCreateProgram(programName, level.id, department?.id);
          if (!program) {
            console.log(`⚠️ Warning: Could not find or create program for ${programName}`);
          } else {
            console.log(`  Program ID: ${program.id}, Program Name: ${program.name}`);
          }
        }
        ...
        if (classroomName) {
          ...
          if (!classroom) {
            console.log(`⚠️ Warning: Could not find or create classroom for ${classroomName}`);
          } else {
            console.log(`  Classroom ID: ${classroom.id}, Classroom Name: ${classroom.name}`);
          }
        } else {
          console.log(`⚠️ No classroom name specified for student ${studentId}`);
        }
    ...
    for (let i = 0; i < studentData.length; i++) {
      try {
        ...
        successCount++;
        console.log(`✅ Created student: ${student.username} (ID: ${createdUser.id})`);
        ...
        console.error(`⚠️ Error creating student #${i + 1} (${studentData[i]?.username}): ${error.message}`);
        errorCount++;
      }
    }
...
async function findOrCreateLevel(levelName: string) {
  ...
  if (!level) {
    const levelFullName = standardLevelName === 'ปวช.' 
...
async function findOrCreateDepartment(departmentName: string) {
  ...
  if (!department) {
    ...
    const departmentId = `D${(departmentCount + 1).toString().padStart(3, '0')}`;
 ...
 */
async function findOrCreateProgram(programName: string, levelId: string, departmentId?: string) {
  ...
  console.log(`Finding program with name: ${standardName}, levelId: ${levelId}`);
  ...
  if (!program) {
    console.log(`No exact match for program ${standardName}, trying with base name...`);
    ...
    if (!program && standardName.includes('ทวิภาคี')) {
      ...
      if (program) {
        console.log(`Found regular program ${program.name} for ทวิภาคี version ${standardName}`);
        ...
        console.log(`Created new ทวิภาคี program: ${standardName} based on ${existingProgramData.name}`);
      }
      ...
      if (!program && !standardName.endsWith('(ทวิภาคี)')) {
        ...
        if (program) {
          console.log(`Found ทวิภาคี program ${program.name} for regular version ${standardName}`);
        }
      }
    }
  }
  ...
  if (!program) {
    ...
    console.log(`Updated program ${standardName} with department ID ${departmentId}`);
  }
 ...
 */
async function findOrCreateClassroom(classroomName: string, levelId?: string, programId?: string, departmentId?: string) {
  ...
  if (!classroom) {
    ...
    if (levelId && classroom.levelId !== levelId) {
      ...
      console.log(`Classroom ${standardName} (ID: ${classroom.id}): Correcting levelId from ${classroom.levelId} to ${levelId}`);
    }
    ...
    if (programId && classroom.programId !== programId) {
      ...
      console.log(`Classroom ${standardName} (ID: ${classroom.id}): Correcting programId from ${classroom.programId} to ${programId}`);
    }
    ...
    if (departmentId && !classroom.departmentId) {
      ...
      console.log(`Classroom ${standardName} (ID: ${classroom.id}): Adding departmentId ${departmentId}`);
    }
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      console.log(`Updated classroom ${standardName} with new/corrected/added IDs`);
    }
  }
