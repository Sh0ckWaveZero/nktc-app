...
async function redistributeStudents() {
  try {
    ...
    for (let i = 0; i < Math.min(5, classrooms.length); i++) {
      ...
      console.log(
        `${classroom.name || classroom.classroomId}: ${
    ...
    if (overloadedClassroom._count.student <= 100) {
      console.log(
        ...
        overloadedClassroom._count.student,
        'students.',
      );
      return;
    }
    ...
    console.log(
      `Found ${studentsToRedistribute.length} students to redistribute`,
    );
    ...
    console.log(
      `Grouped students into ${
        Object.keys(studentGroups).length
      } distinct groups`,
    );
    ...
    for (const [groupKey, students] of Object.entries(studentGroups)) {
      ...
      console.log(
        `\nProcessing group with ${students.length} students (Program: ${
          representative.program?.name || 'unknown'
        }, Department: ${
          representative.department?.name || 'unknown'
        }, Level: ${representative.level?.levelName || 'unknown'})`,
      );
      ...
      if (!targetClassroom) {
        ...
        newClassroomName += `${Math.floor(existingCount / 10) + 1}/${
          (existingCount % 10) + 1
        }`;
        ...
        console.log(
          `Creating new classroom: "${newClassroomName}" (${newClassroomId})`,
        );
      ...
      if (targetClassroom) {
        console.log(
          `Moving ${students.length} students to classroom: "${
            targetClassroom.name || targetClassroom.classroomId
          }"`,
        );
    ...
    const currentCount =
      updatedClassrooms.find((c) => c.id === overloadedClassroom.id)?._count
        .student || 0;
    const reductionPercent = Math.round(
      ((initialCount - currentCount) / initialCount) * 100,
    );

    console.log(
      `\nReduction in overloaded classroom: ${
        initialCount - currentCount
      } students (${reductionPercent}% decrease)`,
    );
  } catch (error) {
