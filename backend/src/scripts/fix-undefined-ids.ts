...
async function fixUndefinedIds() {
  try {
    ...
    console.log(`Students without Program ID: ${studentsWithoutProgramBefore} (${Math.round(studentsWithoutProgramBefore/totalStudents*100)}%)`);
    console.log(`Students without Classroom ID: ${studentsWithoutClassroomBefore} (${Math.round(studentsWithoutClassroomBefore/totalStudents*100)}%)`);
    ...
    if (!fallbackProgram) {
      ...
      console.log(`Fallback program will be: ${fallbackProgram.name || fallbackProgram.id}`);
    }
    ...
    if (!fallbackClassroom) {
      ...
      console.log(`Fallback classroom will be: ${fallbackClassroom.name || fallbackClassroom.id}`);
    }
    ...
    for (const [index, student] of studentsWithIssues.entries()) {
      if (index % 50 === 0) {
        console.log(`Progress: ${index}/${studentsWithIssues.length} students processed`);
      }
      
      // Display student info
      const studentName = `${student.user?.account?.firstName || ''} ${student.user?.account?.lastName || ''}`.trim();
      const studentId = student.studentId || student.user?.username || 'Unknown';
      ...
      if (!student.programId && student.department) {
        try {
          ...
          const levelName = student.level?.levelName as 'ปวช.' | 'ปวส.' || 'ปวช.';
          ...
          console.error(`Error fixing program for student ${studentId}:`, error);
        }
      }
      ...
      if (!student.classroomId) {
        try {
          ...
          const classroomMatches = student.department && student.level 
          ...
          console.error(`Error fixing classroom for student ${studentId}:`, error);
        }
      }
    ...
    console.log('\nResults after fix:');
    console.log(`Students without Program ID: ${studentsWithoutProgramAfter} (${Math.round(studentsWithoutProgramAfter/totalStudents*100)}%)`);
    console.log(`Students without Classroom ID: ${studentsWithoutClassroomAfter} (${Math.round(studentsWithoutClassroomAfter/totalStudents*100)}%)`);
    ...
    const possibleFixes = studentsWithoutProgramBefore + studentsWithoutClassroomBefore;
    
    if (possibleFixes > 0) {
      console.log(`Overall fix success rate: ${Math.round(totalFixed/possibleFixes*100)}%`);
    }
