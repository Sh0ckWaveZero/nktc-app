 ...
 */
async function analyzeStudentsWithoutClassroom(fileName: string, outputFile: string = 'classroom-analysis.json') {
  console.log(`=== Starting Analysis for Students Without Classroom from ${fileName} ===\n`);
  ...
  try {
    ...
    for (const item of studentData) {
      ...
      const existingClassroom = existingClassrooms.find(c => c.name === standardClassroomName);
      ...
      if (!existingClassroom) {
        ...
        analysis.problemDetails.push({
          ...
          levelName: levelName || 'Not specified',
          departmentName: departmentName || 'Not specified',
          possibleMatches: findSimilarClassrooms(standardClassroomName, existingClassrooms)
        });
      }
      ...
      try {
        ...
        if (studentInDb && !studentInDb.classroomId) {
          ...
          analysis.problemDetails.push({
            ...
            classroomName: standardClassroomName,
            programName: programName || studentInDb.program?.name || 'Not specified',
            programIdInDb: studentInDb.programId || null,
            levelName: levelName || studentInDb.level?.levelName || 'Not specified',
            levelIdInDb: studentInDb.levelId || null,
            departmentName: departmentName || studentInDb.department?.name || 'Not specified',
            departmentIdInDb: studentInDb.departmentId || null
          });
        }
      } catch (error) {
        console.log(`Error checking student ${studentId} in database: ${error.message}`);
      }
    }
    ...
    console.log(`- Students without classroom: ${analysis.studentsWithoutClassroom}`);
    console.log(`- Missing classroom names in Excel: ${analysis.missingClassroomNames}`);
    console.log(`- Classroom names in Excel but not in database: ${analysis.classroomNamesButNoId}`);
 ...
 */
function findSimilarClassrooms(classroomName: string, existingClassrooms: any[]) {
  ...
  const level = parts.find(p => p.includes('ปวช') || p.includes('ปวส'))?.trim() || '';
  ...
  return existingClassrooms
    .filter(c => {
      ...
      return (level && name.includes(level)) || 
 ...
 */
async function fixStudentsWithoutClassroom(analysisFile: string, autoFix: boolean = false) {
  try {
    // อ่านผลวิเคราะห์จากไฟล์
    const analysis = JSON.parse(fs.readFileSync(analysisFile, 'utf8'));
    
    console.log(`=== Starting fix for ${analysis.studentsWithoutClassroom} students without classroom ===\n`);
    ...
    for (const problem of analysis.problemDetails) {
      ...
      if (problem.possibleMatches && problem.possibleMatches.length > 0) {
        ...
        console.log(`  Best match in database: ${bestMatch.name} (ID: ${bestMatch.id})`);
        ...
        if (autoFix) {
          try {
            ...
            if (student) {
              ...
              console.log(`  ✅ Fixed: Assigned to classroom ${bestMatch.name}`);
              fixedCount++;
            } else {
              console.log(`  ⚠️ Cannot fix: Student not found in database`);
              unfixable.push({...problem, reason: 'Student not found in database'});
            }
          } catch (error) {
            console.log(`  ❌ Error fixing student ${problem.studentId}: ${error.message}`);
            unfixable.push({...problem, reason: `Database error: ${error.message}`});
          }
        } else {
          console.log(`  ℹ️ Would fix by assigning to classroom ${bestMatch.name}`);
        }
      } else {
        console.log(`Student ${problem.studentId}: ${problem.name}`);
        console.log(`  Problem: ${problem.problem}`);
...
if (require.main === module) {
  ...
  const outputFile = `classroom-analysis-${new Date().toISOString().split('T')[0]}.json`;
