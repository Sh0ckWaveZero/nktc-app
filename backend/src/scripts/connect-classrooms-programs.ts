...
async function connectClassroomsToPrograms() {
  try {
    ...
    console.log(
      `Classrooms without Program: ${classroomsWithoutProgram}/${totalClassrooms} (${Math.round(
        (classroomsWithoutProgram / totalClassrooms) * 100,
      )}%)`,
    );

    if (classroomsWithoutProgram === 0) {
      console.log(
        'All classrooms are already connected to programs. Nothing to do!',
      );
      return;
    }
    ...
    console.log(
      `Found ${classroomsToFix.length} classrooms without program connections`,
    );
    ...
    for (const classroom of classroomsToFix) {
      ...
      if (classroom.student.length > 0) {
        ...
        for (const student of classroom.student) {
          if (student.programId) {
            programCounts[student.programId] =
              (programCounts[student.programId] || 0) + 1;
          }
        }
        ...
        if (mostCommonProgram) {
          ...
          const programName =
            programs.find((p) => p.id === mostCommonProgram)?.name || 'Unknown';
          console.log(
            `  Found matching program "${programName}" from student data (${maxCount} students)`,
          );
        }
      }
        ...
        if (nameParts.length > 1) {
          ...
          for (const program of programs) {
            ...
            if (
              qualifier &&
              program.name &&
              program.name.includes(`(${qualifier})`)
            ) {
              if (
                program.name.includes(potentialProgramName) ||
                potentialProgramName.includes(
                  program.name.replace(`(${qualifier})`, '').trim(),
                )
              ) {
                matchingProgramId = program.id;
                console.log(
                  `  Matched program "${program.name}" by classroom name with qualifier (${qualifier})`,
                );
                break;
              }
            }
            // If no qualifier match or no qualifier exists, try regular matching
              ...
              matchingProgramId = program.id;
              console.log(
                `  Matched program "${program.name}" by classroom name pattern`,
              );
              break;
            }
          }
      ...
      if (!matchingProgramId && classroom.departmentId && classroom.levelId) {
        ...
        if (departmentProgram) {
          ...
          console.log(
            `  Matched program "${departmentProgram.name}" by department and level`,
          );
        } else {
          ...
          if (deptOnlyProgram) {
            ...
            console.log(
              `  Matched program "${deptOnlyProgram.name}" by department only`,
            );
          }
        }
      }
    ...
    console.log(
      `Classrooms without Program: ${classroomsWithoutProgramAfter}/${totalClassrooms} (${Math.round(
        (classroomsWithoutProgramAfter / totalClassrooms) * 100,
      )}%)`,
    );
    ...
    console.log(`\nImprovement summary:`);
    console.log(
      `- Connected ${fixCount} of ${classroomsWithoutProgram} classrooms to programs (${fixPercent}%)`,
    );
  } catch (error) {
