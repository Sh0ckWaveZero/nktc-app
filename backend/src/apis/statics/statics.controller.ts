...
export class StaticsController {
  ...
  @HttpCode(HttpStatus.OK)
  async getTeacherAvatar(@Param('id') id: string, @Res() response: Response) {
    await this.serveImage(id, 'avatars/teachers', response);
  }

  @Get('goodness-individual/images/:id')
  @HttpCode(HttpStatus.OK)
  async getStudentGoodness(@Param('id') id: string, @Res() response: Response) {
    await this.serveImage(id, 'goodness-individual/images', response);
  }

  @Get('badness-individual/images/:id')
  @HttpCode(HttpStatus.OK)
  async getStudentBadness(@Param('id') id: string, @Res() response: Response) {
  ...
  private async serveImage(
    ...
    try {
      ...
      const dataStream = await this.staticsService.getAvatar(
        bucketName,
        `${prefix}/${id}`,
      );
