...
import { classroom, classroomToTeacher, teacher, user, accounts, course, program, reportCheckIn, session, department, levelClassroom, rolePermission, userRole, classroomToLevelClassroom, student, level, activityCheckInReport, verificationtokens } from "./schema";

export const classroomToTeacherRelations = relations(classroomToTeacher, ({one}) => ({
...
export const levelClassroomRelations = relations(levelClassroom, ({one, many}) => ({
	teachers: many(teacher),
	classroomToLevelClassrooms: many(classroomToLevelClassroom),
	students: many(student),
	level: one(level, {
...
export const rolePermissionRelations = relations(rolePermission, ({many}) => ({
	userRoles: many(userRole),
}));

export const classroomToLevelClassroomRelations = relations(classroomToLevelClassroom, ({one}) => ({
	classroom: one(classroom, {
...
export const activityCheckInReportRelations = relations(activityCheckInReport, ({one}) => ({
	classroom: one(classroom, {
...
export const verificationtokensRelations = relations(verificationtokens, ({many}) => ({
	users: many(user),
}));