...
interface ClassroomOption {
  ...
  level?: { id?: string | null; levelName?: string | null } | null;
}
...
const StudentProfileUpload = ({ imgSrc, isCompressing, onImageChange, onReset }: StudentProfileUploadProps) => (
      ...
      <Box sx={{ minWidth: 0, flex: 1 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, mt: 2.5 }}>
          ...
          <Button
            ...
            variant='outlined'
            onClick={onReset}
            sx={{ minHeight: 48, borderRadius: `${CONTROL_RADIUS}px`, textTransform: 'none', px: 2.5, fontWeight: 700 }}
          >
            รีเซ็ต
          </Button>
        </Box>
...
interface StudentPersonalInfoSectionProps {
  ...
  trailItems: readonly any[];
  onStudentIdChange: (value: string, fieldOnChange: (v: string) => void) => void;
}
...
const StudentPersonalInfoSection = ({
  ...
  <>
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[0]}>
        ...
        <Controller
          ...
          render={({ field }) => (
            <RequiredTextField
              ...
              sx={FORM_CONTROL_SX}
              onChange={(e) => onStudentIdChange(e.target.value, field.onChange)}
              slotProps={{ htmlInput: { maxLength: 11, onKeyDown: (e: KeyboardEvent) => handleKeyDown(e) } }}
            />
          )}
        />
      </FormControl>
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[1]}>
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[2]}>
      <FormControl fullWidth>
        <Controller
          name='firstName'
          control={control}
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[3]}>
      <FormControl fullWidth>
        <Controller
          name='lastName'
          control={control}
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[4]}>
      <FormControl fullWidth>
        <Controller
          name='classroom'
          control={control}
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <RequiredTextField
                  ...
                  helperText={(errors.classroom as FieldError | undefined)?.message}
                  sx={FORM_CONTROL_SX}
                />
              )}
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[5]}>
      <FormControl fullWidth>
        <Controller
          name='idCard'
          control={control}
          render={({ field }) => (
            <TextField
              ...
              sx={FORM_CONTROL_SX}
              slotProps={{ htmlInput: { maxLength: 13, onKeyDown: (e: KeyboardEvent) => handleKeyDown(e) } }}
            />
          )}
        />
      </FormControl>
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[6]}>
      <FormControl fullWidth>
        <Controller
          name='birthDate'
          control={control}
            ...
            <ThaiDatePicker
              ...
              slotProps={{
                textField: {
                  fullWidth: true,
                  sx: FORM_CONTROL_SX,
                  input: { endAdornment: <FcCalendar fontSize='1.5rem' /> },
                },
              }}
            />
          )}
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[7]}>
      <FormControl fullWidth>
        <Controller
          name='phone'
          control={control}
          render={({ field }) => (
            <TextField
              ...
              sx={FORM_CONTROL_SX}
              slotProps={{ htmlInput: { maxLength: 10, onKeyDown: (e: KeyboardEvent) => handleKeyDown(e) } }}
            />
          )}
        />
      </FormControl>
    ...
    <AnimatedGrid size={{ xs: 12, sm: 6 }} style={trailItems[8]}>
...
interface StudentAddressSectionProps {
  control: any;
  errors: any;
  currentAddress: ThailandAddressValue;
  onAddressChange: (val: ThailandAddressValue) => void;
}

const StudentAddressSection = ({ control, errors, currentAddress, onAddressChange }: StudentAddressSectionProps) => {
    ...
    <>
      ...
      <ThailandAddressTypeahead value={currentAddress} onValueChange={onAddressChange}>
        ...
        <style>{`
          ...
          .sub-district-input::placeholder, .district-input::placeholder, .province-input::placeholder, .postal-code-input::placeholder {
            color: ${theme.palette.text.secondary};
            opacity: 0.55;
            font-weight: 400;
          }
        `}</style>
        <ThailandAddressTypeahead.Suggestion optionItemProps={{ style: { cursor: 'pointer', borderRadius: '10px' } }} />
      </ThailandAddressTypeahead>
    </>
  );
};
...
const StudentFormActions = ({ isDirty, isValid, isCheckingStudentId, onReset }: StudentFormActionsProps) => (
...
const StudentAddPage = () => {
  ...
  const classroom = useMemo<ClassroomOption[]>(() => {
    if (!classroomsData?.length) return [];
    if (user?.role?.toLowerCase() === 'admin') return classroomsData as ClassroomOption[];
    return classroomsData.filter((item: ClassroomOption) => user?.teacherOnClassroom?.includes(item.id));
  }, [classroomsData, user]);
  ...
  const { handleInputImageChange, isCompressing } = useImageCompression((dataUrl) => setImgSrc(dataUrl));
  ...
  const onSubmit = async (data: StudentAddFormData) => {
    if (errors.studentId || !data.classroom) return;
    ...
    const student = {
      ...
      avatar: imgSrc === DEFAULT_IMG ? null : imgSrc,
    };

    const toastId = toast.info('กำลังบันทึกข้อมูล...', { autoClose: false, hideProgressBar: true });
    createStudent(
        ...
        >
          <CardHeader
            ...
            sx={{
              ...
              '& .MuiCardHeader-avatar': { mr: { xs: 1.75, sm: 2.25 } },
