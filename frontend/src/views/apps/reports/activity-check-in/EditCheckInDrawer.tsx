...
const SidebarEditCheckInDrawer = (props: SidebarAddClassroomType) => {
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>แก้ไขเช็คชื่อการเข้าร่วมกิจกรรม</Typography>
      ...
      <form onSubmit={(event) => onSubmitted(event, { isCheckInStatus: values, data })}>
        <Stack sx={{ p: 5 }} spacing={5}>
          ...
          <FormControl>
            ...
            >
              <FormControlLabel key={'present'} value='present' control={<Radio color='success' />} label='เข้าร่วม' />
              <FormControlLabel key={'absent'} value='absent' control={<Radio color='error' />} label='ไม่เข้าร่วม' />
            </RadioGroup>
          </FormControl>
